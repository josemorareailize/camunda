...
class BatchOperationRetryHandlerTest {
  ...
  void setUp() {
    ...
    context = new RetryContext(BATCH_OPERATION_KEY, SEARCH_CURSOR, 0);
  }
  ...
  void shouldReturnSuccessWhenOperationSucceeds() {
    ...
    // when
    final var result = retryHandler.executeWithRetry(operation, context);

    // then
    assertThat(result).isInstanceOf(RetryResult.Success.class);
    final var success = (RetryResult.Success) result;
    assertThat(success.batchOperationKey()).isEqualTo(BATCH_OPERATION_KEY);
  ...
  void shouldRetryWhenOperationFailsWithRetryableException() {
    ...
    assertThat(retry.context().batchOperationKey()).isEqualTo(BATCH_OPERATION_KEY);
    assertThat(retry.context().searchResultCursor()).isEqualTo("test-end-cursor");
    assertThat(retry.context().numAttempts()).isEqualTo(1);
  }
  ...
  void shouldFailImmediatelyWhenMaxRetriesExceeded() {
    ...
    when(operation.execute()).thenThrow(initException);
    final var maxAttemptsContext =
        new RetryContext(BATCH_OPERATION_KEY, SEARCH_CURSOR, MAX_RETRIES);

    // when
    final var result = retryHandler.executeWithRetry(operation, maxAttemptsContext);
